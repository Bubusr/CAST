...
class CASTPipeline:
    ...
    def __init__(self, output_dir: Optional[Path] = None, 
                 mesh_provider: Literal["tripo3d", "trellis", "hunyuan"] = "trellis",
    ...
    def _get_completed_stages(self, output_dir: Path) -> List[str]:
        ...
        completed = []
        stages = ['detection', 'filtering', 'segmentation', 'depth', 'generation', 'mesh', 'pose', 'scene_graph']
        for stage in stages:
    ...
    def run_single_image(self, image_path: str, 
        ...
        # Step 1: Object Detection (bboxes only)
        print("\n" + "="*60)
        print("STEP 1: Object Detection (RAM + Grounding DINO)")
        print("="*60)
        ...
        # Step 1.5: Detection Filtering
        print("\n" + "="*60)
        print("STEP 1.5: Detection Filtering (Qwen-VL)")
        print("="*60)
        ...
        # Step 1.75: Segmentation (SAM)
        print("\n" + "="*60)
        print("STEP 1.75: Object Segmentation (SAM)")
        print("="*60)
        
        if resume and 'segmentation' in completed_stages:
            print("Resuming: Loading segmentation results from previous run...")
            detected_objects = self._load_stage_result('segmentation', run_output_dir)
        else:
            # Run SAM segmentation on filtered objects
            detected_objects = self.detection_module.run_segmentation(
                image,
                detected_objects,
                output_dir=run_output_dir if save_intermediates else None,
                use_sam_hq=False
            ...
            if detected_objects and save_intermediates:
                self._save_stage_result('segmentation', detected_objects, run_output_dir)
