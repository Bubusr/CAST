...
class PyTorchPoseOptimizer(nn.Module):
    ...
    def chamfer_distance_3d(self, source_points: torch.Tensor, target_points: torch.Tensor,
        ...
        losses, loss_normals = chamfer_distance(source, target, 
            x_normals=source_normals, y_normals=target_normals, norm=1, batch_reduction=None, point_reduction=None)
    ...
    def optimize_pose(self, source_points: torch.Tensor, target_points: torch.Tensor,
        ...
        for iteration in range(num_iterations):
            ...
            loss_chamfer = self.chamfer_distance_3d(
            ...
            # add some reg loss for the rotation 
            loss_rot_reg = torch.norm(rotation_matrix - torch.eye(3).type_as(rotation_matrix), p=2) * rot_reg_weight
            loss = loss_chamfer + loss_rot_reg
            ...
            if loss_chamfer.item() < best_loss:
                best_loss = loss_chamfer.item()
                best_translation = translation.clone().detach()
                best_rotation_6d = rotation_6d.clone().detach()
                best_scale = scale.clone().detach()

            if torch.isinf(loss_chamfer) or torch.isnan(loss_chamfer) or torch.isinf(loss_rot_reg) \
                or torch.isnan(loss_rot_reg) or torch.isinf(loss) or torch.isnan(loss):
                print(f"  Iter {iteration:4d}/{num_iterations}: Loss is NaN")
                import ipdb; ipdb.set_trace()
            
            # Print progress
            if verbose and (iteration % (num_iterations // 10) == 0 or iteration == num_iterations - 1):
                print(f"    Iter {iteration:4d}/{num_iterations}: Chamfer Loss = {loss_chamfer.item():.4f}, Rotation Reg Loss = {loss_rot_reg.item():.4f}, Total = {loss.item():.4f}", end='\r')
    ...
    def register_point_clouds(self, mesh_points: np.ndarray, scene_points: np.ndarray,
        ...
        if downsample_target > 0:
            downsample_source_factor = len(source_points) // downsample_target if len(source_points) > downsample_target else 1
            downsample_target_factor = len(target_points) // downsample_target if len(target_points) > downsample_target else 1
            source_points = source_points[::downsample_source_factor]
            target_points = target_points[::downsample_target_factor]
            if source_normals is not None:
                source_normals = source_normals[::downsample_source_factor]
            if target_normals is not None:
                target_normals = target_normals[::downsample_target_factor]
            if source_colors is not None:
                source_colors = source_colors[::downsample_source_factor]
            if target_colors is not None:
                target_colors = target_colors[::downsample_target_factor]
        ...
        try:
            ...
            transformation[:3, :3] = rotation_np * scale_np.reshape(-1, 1)
