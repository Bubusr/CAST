...
class PoseEstimationModule:
    ...
    def _check_existing_pose(self, detected_object: DetectedObject, 
                            output_dir: Optional[Path]) -> Optional[MeshPose]:
        """
        Check if a pose already exists for this object and load it
        
        Args:
            detected_object: Object to check pose for
            output_dir: Directory where poses are saved
            
        Returns:
            Loaded MeshPose if files exist, None otherwise
        """
        if output_dir is None:
            return None
        
        pose_dir = output_dir / "pose_estimation" / f"object_{detected_object.id}"
        if not pose_dir.exists():
            return None
        
        # Check if all required files exist
        required_files = [
            pose_dir / "translation.npy",
            pose_dir / "rotation_matrix.npy",
            pose_dir / "scale.npy"
        ]
        ...
        try:
            # Load pose components
            translation = np.load(pose_dir / "translation.npy")
            rotation = np.load(pose_dir / "rotation_matrix.npy")
            scale = np.load(pose_dir / "scale.npy")
            
            # Load confidence/fitness if available
            fitness_file = pose_dir / "fitness_score.txt"
            if fitness_file.exists():
                with open(fitness_file, 'r') as f:
                    confidence = float(f.read().strip())
            else:
                confidence = 1.0  # Default confidence if not saved
            
            print(f"  Found existing pose for object {detected_object.id} (confidence: {confidence:.3f})")
            ...
            print(f"  Error loading existing pose for object {detected_object.id}: {e}")
            return None
    ...
    def extract_object_point_cloud_from_instance(self, depth_estimation: DepthEstimation,
        ...
        if extract_colors and len(object_points) > 0:
            ...
            valid_indices = ~np.any(np.isinf(masked_points) | np.isnan(masked_points) | (masked_points == 0), axis=1)
    ...
    def estimate_object_pose_torch(self, mesh: Mesh3D, detected_object: DetectedObject,
        ...
        transformation, final_loss, success = self.pytorch_registration(
            mesh_points, mesh_normals if use_normals else None, mesh_colors,
    ...
    def run(self, meshes: List[Optional[Mesh3D]], detected_objects: List[DetectedObject],
        ...
        for i, (mesh, detected_obj) in enumerate(zip(meshes, detected_objects)):
            ...
            # Check if pose already exists (instance-level resuming)
            existing_pose = self._check_existing_pose(detected_obj, output_dir)
            if existing_pose is not None:
                print(f"Skipping pose estimation for object {detected_obj.id}: {detected_obj.description} (already exists)")
                pose = existing_pose
            elif self.backend == "icp":
